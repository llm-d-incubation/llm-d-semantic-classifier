# AC-009 RED evidence — I-005 / I-006 (dummy Praxis semantics)
...
(integration) and S-001/S-002 (OpenShift system). I-001/I-002 (real tonic round
...
This slice selects the dummy-Praxis semantics: I-005 (the dummy Praxis preserves
...
dummy Praxis consumes the ranked signal, then routes OUTSIDE llm-d-sc via its
...
routing/session authority stays Praxis (AC-010), which is the essence of AC-009.

## RED state (feature does not exist)
There is no `dummy_praxis` module in the crate. `src/lib.rs` exposes only
`cache`, `classify`, `config`, `embedding`, `grpc`, `queue`, `ranker`,
`runtime`, `tokenizer`. The proving tests drive a
`llm_d_sc::dummy_praxis::DummyPraxis` client (connect over the persistent
channel, classify_and_route, DummyRequest/DummyOutcome) against the real
classify server. Because `llm_d_sc::dummy_praxis` is undefined, neither proving
...
## Result
FAILED. Expected RED reason: the dummy-Praxis layer (AC-009 I-005/I-006) does
not exist yet, so the proving tests cannot compile, let alone pass.

Failure excerpt:
```
error[E0432]: unresolved import `llm_d_sc::dummy_praxis`
   --> tests/grpc.rs:136:19
    |
136 |     use llm_d_sc::dummy_praxis::{DummyPraxis, DummyRequest};
    |                   ^^^^^^^^^^^^ could not find `dummy_praxis` in `llm_d_sc`

error[E0432]: unresolved import `llm_d_sc::dummy_praxis`
   --> tests/grpc.rs:173:19
    |
173 |     use llm_d_sc::dummy_praxis::{DummyPraxis, DummyRequest};
    |                   ^^^^^^^^^^^^ could not find `dummy_praxis` in `llm_d_sc`
...
AC-009 demands the dummy Praxis consume the classification response over a
persistent gRPC channel and then apply its own routing outside llm-d-sc. The
persistent gRPC layer already exists (I-001/I-002 green), but the dummy-Praxis
...
`llm_d_sc::dummy_praxis` is undefined, `cargo test --locked --test grpc` fails at
exit 101 with "could not find `dummy_praxis` in `llm_d_sc`". This is precisely
the expected RED: the feature (dummy-Praxis semantics) does not exist, so the
proving tests cannot run, let alone pass. The failure is deterministic and
non-vacuous — once a `src/dummy_praxis.rs` exposing `DummyPraxis::connect`,
`DummyPraxis::classify_and_route`, `DummyRequest`, and `DummyOutcome` is
