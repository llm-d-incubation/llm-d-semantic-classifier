...
fn main() {
    ...
    let data_path = arg("--dataset", Some("evals/datasets/sensitivity-heldout.jsonl"));
    let out_path = arg("--json", Some(""));

    let def: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&def_path).expect("read classifier")).unwrap();
    ...
    for item in &items {
        ...
        let ok = pred == truth;
        if ok { correct += 1; } else {
            errors.push((text.to_string(), truth.clone(), pred.clone(), confidence));
        }
        if hard { hard_total += 1; if ok { hard_correct += 1; } }
    ...
    for l in &labels {
        ...
        let fp: f64 = labels.iter()
            .map(|t| *confusion.get(&(t.clone(), l.clone())).unwrap_or(&0) as f64).sum::<f64>() - tp;
        let fneg: f64 = labels.iter()
            .map(|p| *confusion.get(&(l.clone(), p.clone())).unwrap_or(&0) as f64).sum::<f64>() - tp;
        let p = if tp + fp > 0.0 { tp / (tp + fp) } else { 0.0 };
        let r = if tp + fneg > 0.0 { tp / (tp + fneg) } else { 0.0 };
        let f1 = if p + r > 0.0 { 2.0 * p * r / (p + r) } else { 0.0 };
        f1s.push(f1);
        println!("{:<15} {:>9.3} {:>7.3} {:>7.3} {:>9}", l, p, r, f1, (tp + fneg) as usize);
    }
    let macro_f1 = f1s.iter().sum::<f64>() / f1s.len() as f64;

    println!("\nconfusion (rows = truth, cols = predicted)");
    print!("{:<15}", "");
    for l in &labels { print!("{:>13}", &l[..l.len().min(12)]); }
    println!();
    for t in &labels {
        print!("{:<15}", t);
        for p in &labels { print!("{:>13}", confusion.get(&(t.clone(), p.clone())).unwrap_or(&0)); }
        println!();
    }

    latencies_us.sort();
    let p50 = latencies_us[latencies_us.len() / 2] as f64 / 1000.0;
    let p99 = latencies_us[(latencies_us.len() as f64 * 0.99) as usize % latencies_us.len()] as f64 / 1000.0;
    ...
    if hard_total > 0 {
        println!("hard-case acc    {:.4}  ({hard_correct}/{hard_total})",
    ...
    if !errors.is_empty() {
        ...
        for (t, truth, pred, c) in errors.iter().take(25) {
            println!("  {truth} -> {pred} (conf {c:.2})  \"{}\"",
                     &t[..t.len().min(72)]);
        }
    }

    if !out_path.is_empty() {
        let conf_rows: Vec<Vec<usize>> = labels.iter()
            .map(|t| labels.iter().map(|p| *confusion.get(&(t.clone(), p.clone())).unwrap_or(&0)).collect())
            .collect();
